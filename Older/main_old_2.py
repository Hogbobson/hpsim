...
class hpsim:
    ...
    def _record_data(cls, var_key, save_key):
        ...
        for i in cls.ensemble[var_key].flatten():
    ...
    def _step(cls):
        ...
        cls.ensemble['force'] = np.empty_like(
                cls.ensemble['affectee position'])
        temp_f = cls.ensemble['force'].flatten()
        for i in range(temp_f.size):
            temp_f[i] = np.empty((0,3), float)
        cls.ensemble['force'] = np.reshape(temp_f, cls.ensemble['force'].shape)
        ...
        for k in np.arange(0, len(cls.force_dict_keys), 2):
            ...
            force = force_func(*fargs).flatten()
            temp_f = cls.ensemble['force'].flatten()
            for i in range(temp_f.size):
                temp_f[i] = np.append(temp_f[i], force[i], axis = 0)
        
        n_forces = len(cls.force_dict_keys)//2
        n_parts = cls.ensemble['number of objects']
        for i in range(temp_f.size):
            temp_f[i] = np.reshape(temp_f[i], (n_parts, 3, n_forces))
        cls.ensemble['force'] = np.reshape(temp_f, cls.ensemble['force'].shape)
...
# =============================================================================
        ...
        cls.ensemble['acceleration'] = cls._acceleration(
                cls.ensemble, *cls._acceleration_args)
    ...
    def preload_solar_system(cls):
        ...
        """
        cells, lims = arena.make_arena_inf()
        ...
        cls.set_ensemble_generator(ensgen.solar_system, (cells, lims))
        cls.set_integration_method(integrator.n_body_nice)
        cls.set_solver(solver.sym1, (cls.time_step,))
        cls.set_ensemble()
        cls._ready_to_go = True
        if not cls.mute:
    ...
    def preload_n_body_nice(cls):
        ...
        cls.set_acceleration_function(acceleration.classic_acceleration)
        cls.set_wanted_forces(cls._wanted_forces)
        cls.set_ensemble_generator(ensgen.n_body_nice, (cells, lims))
        cls.set_integration_method(integrator.n_body_nice)
        cls.set_solver(solver.sym1, (cls.time_step,))
        cls.set_ensemble()
        cls._ready_to_go = True
    ...
    def preload_lennard_jones(cls):
        cls._wanted_forces = [force.lennard_jones]
        cls.set_time(0, timestep.constant_dt(1), timegen.time_minutes(100))
        cls.set_acceleration_function(acceleration.classic_acceleration)
        cls.set_wanted_forces(cls._wanted_forces)
        cls.set_ensemble_generator(ensgen.lennard_jones_example)
        cls.set_integration_method(func = \
                                   integrator.ParticleInCell.particle_in_cell,
                                   class_name = integrator.ParticleInCell,
                                   class_args = (60, 120))
        cls.set_solver(solver.sym1, (cls.time_step,))
        cls.set_ensemble()
        cls._ready_to_go = True
    ...
    def set_acceleration_function(cls,func, args = (), 
...
# =============================================================================
    ...
    def set_integration_method(cls, func, args = (), 
        ...
        if class_name:
            ...
            cls._integration_args = (cls._integration_object,)
        else:
            cls._integration_method = func
    ...
    def set_wanted_forces(cls, funcs):
        ...
        for f in funcs:
            ...
            if standard_force_params:
                ...
                Warning('The force' + str(f) + 'is not in the standard\
                        library. you should call main.set_force_dict\
                        yourself.')
        cls._force_variables = miscfuncs.get_force_variables(funcs)
    ...
    def runtime_test(cls):
        backup = dict(cls.ensemble)
        tstart = time.time()
        while cls._iteration_steps < 10:
            cls._step()
        tend = time.time()
        cls._iteration_steps = 0
        cls.ensemble = dict(backup)
        telaps = tend - tstart
        tpass  = cls.time_step*10
        texp   = (cls.time_end - cls.time_start)/tpass * telaps
        if telaps < 0.1:
            telaps *= 1000
            unit1   = 'milliseconds'
        else:
        ...
        print("")
        print("10 iterations took {0:2.2f} {1}.".format(telaps, unit1))
        print("Expected runtime: {0:2.2f} {1}.".format(texp, unit2))
        print("")
...
#         cls.time_start          = 0
#         cls.time_step           = timestep.constant_dt(1e6)
#         cls.time_end            = timegen.time_years(50)
...
#         
#         # First let the integrator make sure everything is in order
#         cls._integrator(cls.ensemble, *cls._integrator_args)
#         
#         # Then make an empty force list
#         cls.force = []
#         # Iterate over all forces
...
#         while cls._time_now < cls.time_end:
#             cls.step()
#             cls.record_data('affectee position', 'affectee position data')
